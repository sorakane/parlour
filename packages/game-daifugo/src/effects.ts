...
import { combination, reversed, spadeReturn } from './combinations';
...
export function playEffects(
  ...
  const count = (rank: number) =>
    enabled ? cards.filter((card) => !isJoker(card) && orderOf(card) === rank).length : 0;
  return {
    revolution:
      set.kind === 'run'
    ...
    clearReason: spadeReturn(state, cards)
      ? 'spade-three'
...
export function forbiddenFinishReason(
  ...
  if (
    ...
    cards.some((card) => !isJoker(card) && orderOf(card) === strongest)
  )
    return '最強札上がり';
  return null;
}
