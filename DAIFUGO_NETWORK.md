...
待機部屋は主催者の短いアプリ切り替えを切断と誤認しないよう、応答待ちを120秒にする。
接続開始に10秒間進展がない場合は再試行し、参加画面は席の割り当てを最大90秒待つ。
ページの表示復帰・オンライン復帰ではNostrのソケットと購読を開き直す。
タブの完全終了やOSによるページ破棄は別で、常時稼働サーバーの代わりにはならない。

`ROOM_TEST_URL=http://127.0.0.1:4321 ROOM_TEST_FORCE_RELAY=1 node scripts/verify-mobile-lobby.cjs`

名前付きの4人参加、主催者のブラウザ停止・復帰、ルール共有、6手の同期を確認する。
Chromeのライフサイクル制御による検証であり、iOS実機の停止動作そのものではない。
